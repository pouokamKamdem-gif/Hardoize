...
const creerTables = async () => {
  ...
  `);
    // Table historique des ventes (panier complet par transaction)
await db.execAsync(`
  CREATE TABLE IF NOT EXISTS historique_ventes (
    id            INTEGER PRIMARY KEY AUTOINCREMENT,
    panierJson    TEXT NOT NULL,
    typePaiement  TEXT NOT NULL,
    montantTotal  REAL NOT NULL,
    clientId      INTEGER,
    nomClient     TEXT,
...
// ══════════════════════════════════════════════════════════════
//  HISTORIQUE VENTES (panier complet par transaction)
// ══════════════════════════════════════════════════════════════

export const HistoriqueDB = {

  /**
   * Enregistre une transaction complète (panier + métadonnées).
   */
  inserer: async (data) => {
    const result = await execute(
      `INSERT INTO historique_ventes
        (panierJson, typePaiement, montantTotal,
         clientId, nomClient, vendeurId, nomVendeur,
         groupeId, syncEnAttente)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`,
      [
        JSON.stringify(data.panier),
        data.typePaiement,
        data.montantTotal,
        data.clientId   || null,
        data.nomClient  || null,
        data.vendeurId  || null,
        data.nomVendeur || null,
        data.groupeId   || 0,
      ]
    );
    return result.lastInsertRowId;
  ...
  /**
   * Récupère l'historique avec filtre.
   * filtre : "tout" | "aujourd_hui" | "hier" | "especes" | "credit"
   * vendeurId : optionnel, pour filtrer par vendeur
   */
  getFiltre: async (groupeId, filtre = "tout", vendeurId = null) => {
    let whereExtra = "";
    ...
    const tsDebutHier = debutHier.getTime();

    switch (filtre) {
      case "especes":
        whereExtra = `AND typePaiement = 'especes'`;
        break;
      case "credit":
        whereExtra = `AND typePaiement = 'credit'`;
        break;
      case "aujourd_hui":
        whereExtra = `AND createdAt >= ${tsDebutJour}`;
        break;
      case "hier":
        whereExtra = `AND createdAt >= ${tsDebutHier} AND createdAt < ${tsDebutJour}`;
        break;
      default:
        whereExtra = "";
    }
    ...
    if (vendeurId) {
      whereExtra += ` AND vendeurId = ${vendeurId}`;
    }

    const rows = await query(
      `SELECT * FROM historique_ventes
       WHERE groupeId = ? ${whereExtra}
       ORDER BY createdAt DESC`,
      [groupeId]
    );

    // Parser le panier JSON pour chaque transaction
    return rows.map((row) => ({
      ...row,
      panier: JSON.parse(row.panierJson),
    }));
  },
};
